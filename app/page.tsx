...
export default function Home() {
  ...
  useEffect(() => {
    hydrateFromUrl();
    setZonePopulationEstimate(decodeZonePopulationEstimate(new URLSearchParams(window.location.search).get('zonePop')));
  }, []);
        ...
        <div>
          ...
          <div className="space-y-1.5">
            ...
            {liveAsteroids.map((a) => {
              const selected = asteroid?.id === a.id;
              return (
                <button key={`${a.source}-${a.id}-${a.dateIso ?? a.date}`} type="button" onClick={() => pickAsteroid(a)}
                  className={`press-feedback w-full rounded-xl border p-3 text-left transition-all ${
                    selected ? 'border-orange-400/60 bg-orange-500/12 ring-1 ring-orange-400/30'
                             : 'border-white/8 bg-white/[0.02] hover:border-white/16 hover:bg-white/[0.04]'}`}>
                  <div className="flex items-start gap-2.5">
      ...
      <div>
        ...
        <div className="mb-2 grid grid-cols-2 gap-1.5">
          {(Object.keys(DENSITY_PRESETS) as DensityPreset[]).map((key) => (
            <button key={key} type="button" onClick={() => { setDensityPreset(key); setCustomDensityPerKm2(null); setZonePopulationEstimate(null); }}
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="rounded-xl border border-red-500/20 bg-red-500/[0.06] p-3 text-center">
            <p className="text-[11px] font-medium text-red-300/80">Estimated fatalities</p>
          ...
          <div className="rounded-xl border border-amber-500/20 bg-amber-500/[0.06] p-3 text-center">
            <p className="text-[11px] font-medium text-amber-300/80">Estimated injuries</p>
        ...
        <p className="mt-2 text-[10.5px] leading-relaxed text-slate-600">
          Ranges include mapped-radius uncertainty{diameterUncertaintyPhrase(asteroid)} and {zonePopulationEstimate ? 'WorldPop population estimates for each effect ring' : `assume ${formatBig(density)} people/km² are evenly spread across each effect zone`}. Real losses depend on shelter, building stock, time of day, terrain, weather, local response, and whether the selected target material matches the real surface.
