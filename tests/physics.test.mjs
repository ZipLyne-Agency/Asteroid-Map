...
test('malformed numeric inputs cannot produce non-finite physics or negative casualties', () => {
  const result = calculateImpact({
    diameter: Number.NaN,
    velocity: Number.POSITIVE_INFINITY,
    angle: Number.NaN,
    density: Number.NaN,
    targetDensity: Number.NaN,
  });
  const numericResults = Object.values(result).filter((value) => typeof value === 'number');
  assert.ok(numericResults.every(Number.isFinite));

  const byDensity = estimateCasualties(result, -100);
  assert.equal(byDensity.totalExposed, 0);
  assert.equal(byDensity.totalDeaths, 0);
  assert.equal(byDensity.totalInjuries, 0);
  ...
  assert.equal(byZone.totalExposed, 0);
  assert.equal(byZone.totalDeaths, 0);
  assert.equal(byZone.totalInjuries, 0);
});
