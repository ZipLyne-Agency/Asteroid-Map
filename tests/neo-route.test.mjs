...
test('JPL CAD parser preserves sub-10-meter H-based diameter estimates', () => {
  const payload = {
    signature: { source: 'NASA/JPL SBDB Close Approach Data API', version: SUPPORTED_CAD_API_VERSION },
    count: 1,
    fields: ['des', 'jd', 'cd', 'dist', 'dist_min', 'dist_max', 'v_rel', 'v_inf', 't_sigma_f', 'h', 'diameter', 'diameter_sigma', 'fullname'],
    data: [[
      '2026 TEST', '2461234.5', '2026-Jan-01 00:00', '0.01', '0.009', '0.011',
      '12', '10', '< 00:01', '30', null, null, '       (2026 TEST)',
    ]],
  };

  const [item] = parseCadPayload(payload, 'upcoming');

  assert.ok(item.diameter > 3 && item.diameter < 4);
  assert.ok(item.diameterMin < item.diameter);
  assert.ok(item.diameterMax > item.diameter);
});

test('JPL CAD parser drops rows whose physical inputs are unavailable', () => {
  const base = ['2026 TEST', '2461234.5', '2026-Jan-01 00:00', '0.01', '0.009', '0.011', '12', '10', '< 00:01', '24', null, null, '       (2026 TEST)'];
  const payload = {
    signature: { source: 'NASA/JPL SBDB Close Approach Data API', version: SUPPORTED_CAD_API_VERSION },
    count: 3,
    fields: ['des', 'jd', 'cd', 'dist', 'dist_min', 'dist_max', 'v_rel', 'v_inf', 't_sigma_f', 'h', 'diameter', 'diameter_sigma', 'fullname'],
    data: [
      base.map((value, index) => index === 9 ? null : value),
      base.map((value, index) => index === 6 || index === 7 ? null : value),
      base.map((value, index) => index === 3 ? null : value),
    ],
  };

  assert.deepEqual(parseCadPayload(payload, 'upcoming'), []);
});
