...
function zonePopulationsFromDensity(
  ...
  const safeDensity = Number.isFinite(densityPerKm2) ? Math.max(0, densityPerKm2) : 0;
  return {
    crater: areas.crater * safeDensity,
    fireball: areas.fireball * safeDensity,
    blast: areas.blast * safeDensity,
    minorBlast: areas.minorBlast * safeDensity,
    thermal: areas.thermal * safeDensity,
  };
}

function estimateForZonePopulations(populations: CasualtyZonePopulations, rate: 'low' | 'central' | 'high') {
  const safePopulation = (value: number) => Number.isFinite(value) ? Math.max(0, value) : 0;
  const craterPopulation = safePopulation(populations.crater);
  const fireballPopulation = safePopulation(populations.fireball);
  const blastPopulation = safePopulation(populations.blast);
  const minorBlastPopulation = safePopulation(populations.minorBlast);
  const thermalPopulation = safePopulation(populations.thermal);
  ...
  const exposed =
    ...
    fireballPopulation +
    blastPopulation +
    minorBlastPopulation +
    thermalPopulation;

  const craterDeaths = craterPopulation;
  const fireballDeaths = fireballPopulation * fireballDeathRate;
  const blastDeaths = blastPopulation * blastDeathRate;
  const blastInjuries = blastPopulation * blastInjuryRate;
  const thermalDeaths = thermalPopulation * thermalDeathRate;
  const thermalInjuries = thermalPopulation * thermalInjuryRate;
  const minorBlastInjuries = minorBlastPopulation * minorBlastInjuryRate;
