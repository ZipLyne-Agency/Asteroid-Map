...
export function parseCadPayload(
  ...
  return payload.data
    .map((row): ParsedApproach | null => {
      ...
      const relativeVelocity = toNumber(row[fieldIndex.get('v_rel') ?? -1], 15);
      ...
      const diameterEstimated = diameterKm <= 0;
      ...
      const missDistanceRange = missDistanceMinKm && missDistanceMaxKm
      ...
      return {
        ...
        ...(missDistanceMinKm ? { missDistanceMinKm } : {}),
        ...(missDistanceMaxKm ? { missDistanceMaxKm } : {}),
